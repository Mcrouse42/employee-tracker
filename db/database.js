...
module.exports = {
  ...
  getAllRole: () =>
    connection.query(
      `SELECT role.id, title, salary, department.name AS department FROM role LEFT JOIN department ON role.department_id = department.id ORDER BY role.id ASC;`
    ),
  getAllEmployee: () =>
    connection.query(
  ...
  addDep: (dep) =>
    connection.query(`INSERT INTO department (name) VALUES (?)`, [dep]),
  addRole: (role, salary, id) =>
    connection.query(
